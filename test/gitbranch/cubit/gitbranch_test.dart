...
void main() {
  group('GitBranch', () {
    test('Create', () {
      const value = GitBranch(uuid: 'uuid', name: 'name', directory: 'directory', origin: 'origin');
    ...
    test('to Json', () {
      ...
      expect(value.toJson(), equals({'uuid': 'uuid', 'name': 'name', 'directory': 'directory', 'origin': 'origin'}));
    });

    test('to String', () {
      const value = GitBranch(uuid: 'uuid', name: 'name', directory: 'directory', origin: 'origin');

      expect(value.toString(), equals('{uuid: uuid, name: name, directory: directory, origin: origin}'));
    });

    test('From Json', () {
      final value = GitBranch.fromJson(const {'uuid': 'uuid', 'name': 'name', 'directory': 'directory', 'origin': 'origin'});

      expect(value, equals(const GitBranch(uuid: 'uuid', name: 'name', directory: 'directory', origin: 'origin')));
    });
  });
}
